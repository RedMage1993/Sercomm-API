...
Specifically, when setting up the iCamera2, you need to hold down the WPS reset button for a long time. The WPS button is located on the back of the camera and the light on the bottom right in the front is the WPS indicator. A diagram of this can be found in the manual in this fork. Search for "WPS." It is a bit difficult because you do have to apply a fair amount of pressure. A user in the above thread mentioned that they couldn't log in at first to enable the UI (/adm/enable_ui.cgi) because the username and password was incorrect. It turns out that they needed to hold down the WPS longer in order to reset. I don't know if you need to hold it for one minute, or if it is just let go of 30 seconds. One one of my cameras, I held it for about a full minute and noticed all the lights turn off. On a different camera, I struggled to hold the WPS button down long enough and I think I lifted my finger around 30 seconds, at which pointed it seemed to turn all the lights off and reset. It will then power back on and the bottom left light will begin to flash. I logged into my router and went to the WPS page to add a device. It told me to go press the WPS button on the device I wanted to add, and so I simply pressed the WPS on the back of the iCamera2. You can see the WPS light flashing on the iCamera2 and then in the center, indicating that the connection is in progress and that there is activity. The page on my router eventually said that the connection was successful. You can connect via WPS in other ways, e.g. holding it down for a few seconds on the iCamera2 and then pressing the button on your router. I just liked having the UI on the router give me feedback about the connection attempt. You may also connect the camera by Ethernet and configure it to use your WiFi in that way, but you will need to use a PoE Switch or Injector since the iCamera2 only receives power over Ethernet (PoE). I then went to the DHCP client list on my router to grab the IP addresses of my cameras. After enabling their UI, I was able to go to their IP addresses directly and log in to both and configure them as needed. The manual for the iCamera2 says that you need to hold it for over 10 seconds to reset, but that definitely did not work for me. Therefore, I'd recommend just trying to hold it for at least 1 minute and then let go. You'll know it has reset when the lights turn off.
